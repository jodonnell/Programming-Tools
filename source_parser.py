...
class SourceParser():
    ...
    def _get_classes(self):
        class_nodes = compiler.walk(self.source, ClassVisitor()).class_nodes

        num_props = []
        for class_node in class_nodes:
            num_props.append(compiler.walk(class_node, PropertyVisitor()).num_props)

        self._create_source_classes(class_nodes, num_props)
        self._inheritance(class_nodes)

    def _create_source_classes(self, class_nodes, num_props):
        self.source_classes = []
        for source_class in zip(class_nodes, num_props):
            self.source_classes.append(SourceClass(source_class[0].name, source_class[1]))

    def _inheritance(self, class_nodes):
        for class_node in class_nodes:
